...
This document freezes the technical contract for the daily **`ga4_public_dataset.csv`** drops that arrive in **`gs://platform_assignment_bucket/ga4_raw/YYYY/MM/DD/ga4_public_dataset.csv`** at 07:00 CET. It captures the initial profiling results, establishes a schema baseline, and lists the quality rules that the automated ingestion pipeline will enforce.
...
| Check                 | Result                                           | Notes                                                        |
| --------------------- | ------------------------------------------------ | ------------------------------------------------------------ |
| **Row count**         | **416 097 data rows + 1 header = 416 098 total** | Output from `wc ‑l`. Future runs ± 5 % warning, ± 20 % fail. |
| **Encoding**          | ASCII (valid subset of UTF‑8)                    | Verified with `file` utility; no BOM detected.               |
| **Delimiter**         | Comma `,`                                        | Confirmed by inspecting first 10 lines.                      |
| **Quote char**        | Double quote `"`                                 | Required because JSON blobs are embedded.                    |
| **Embedded newlines** | *Present* inside quoted fields                   | Loader flag `--allow_quoted_newlines=true` is mandatory.     |
| **Compression**       | None                                             | gzip allowed in future versions.                             |

### 2.1 Row‑count sanity
...
### 2.4 Outlier quote patterns

```shell
# Look for unquoted "{" that might break loader
gsutil cat gs://platform_assignment_bucket/ga4_public_dataset.csv \
  | grep -n --color=auto '{' | grep -v '"{'
```
...
These matches confirm multi‑line JSON within quoted fields; we load with `--allow_quoted_newlines=true`.

---

## 3 Schema baseline & contract storage

*Initial load into temporary dataset* `mozaffar_kazemi_scratch`:
...
*Persisted contract JSON in GCS* (`contracts/Mozaffar_Kazemi_GA4Schema.json`):
...
22 column contract (must not change without PR)</summary>
...
Full JSON in GCS at `contracts/Mozaffar_Kazemi_GA4Schema.json`.
